...
def do_complete(backend, login, user=None, redirect_name='next',
    ...
    # 过期时间  十天
    response.set_cookie("username", user.username if user.username else user.email, max_age=10*24*3600)
    response.set_cookie("token", jwt_encode_handler(payload), max_age=10*24*3600)
    return response
